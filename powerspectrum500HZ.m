    ...
    for q=1:length(sub1dir)
        if( isequal( sub1dir(q).name, '.' )||...
          ...
          for j=1:24 
              ...
              wake=periodshour(xwake,1:515);%把wake的行数提出来
...
%               wake1(m,:)=[];
              ...
              nrem=periodshour(xnrem,1:515);%把nrem的行数提出来
...
%               wake1(m,:)=[];
              ...
              rem=periodshour(xrem,1:515);%把rem的行数提出来
...
%               rem1(m,:)=[];
              ...
              microarousal=periodshour(xmicroarousal,1:515);%把microarousal的行数提出来
